...
до 255 вариантов на вопрос; описания могут быть строками, JSON-объектами,
массивами или `null`. Объекты и массивы копируются и проверяются на допустимые
JSON-значения, но их внутренним полям адаптер не задаёт собственный смысл.
Адаптер проверяет, что ответ содержит ровно настроенные классы и сумма
вероятностей близка к 1.
